...
# ─── KONFIGURATSIYA ────────────────────────────────────────
BOT_TOKEN  = os.environ.get("BOT_TOKEN")  or "8915703947:AAH9KFMBfoC_iUZIclbto9Gr3eIkpyKeeho"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Inline tugmalar: Kod  |  Kanal  (va Qo'llanma)
    # "Kod" tugmasi — kino_kanal_url yoki birinchi majburiy kanal URL ga yo'naltiradi
    kanal_url_for_kod = ""
    if RAM.channels:
        first_ch = RAM.channels[0]
        kanal_url_for_kod = first_ch.get("url") or channel_join_url(first_ch.get("username", ""), "")
    if not kanal_url_for_kod:
        kanal_url_for_kod = RAM.settings.get("kino_kanal_url", "")

    if kanal_url_for_kod:
        kod_btn = ibtn(bt("kod_btn"), url=kanal_url_for_kod, style="primary",
    ...
    # "Kanal" tugmasi — faqat alohida kanal URL bo'lsa (kod tugmasidan farq qilsa)
    if RAM.channels:
        first_ch = RAM.channels[0]
        ch_url = first_ch.get("url") or channel_join_url(first_ch.get("username", ""), "")
        if ch_url and ch_url != kanal_url_for_kod:
            start_inline_rows[0].append(
                ibtn(bt("kanal_btn"), url=ch_url, style="primary",
                     emoji_id=get_eid("kanal_btn"))
            )
    elif RAM.settings.get("kino_kanal_url") and RAM.settings["kino_kanal_url"] != kanal_url_for_kod:
        start_inline_rows[0].append(
            ibtn(bt("kanal_btn"), url=RAM.settings["kino_kanal_url"],
                 style="primary", emoji_id=get_eid("kanal_btn"))
        )
