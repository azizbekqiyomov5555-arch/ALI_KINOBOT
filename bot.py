...
def is_super_admin(uid) -> bool:
    """Faqat asosiy admin (.env dagi ADMIN_ID)."""
    try:
...
def is_admin(uid) -> bool:
    """Asosiy admin yoki qo'shilgan adminlar."""
    try:
        u = int(uid)
    except Exception:
        return False
    if u == ADMIN_ID:
        return True
    try:
        return u in RAM.admins
    except Exception:
        return False

JSONBLOB_URL      = os.environ.get("JSONBLOB_URL") or "https://jsonblob.com/api/jsonBlob/018df4aa-10b1-735c-83f5-9901ab2db9b6"
GSHEET_ID         = os.environ.get("GSHEET_ID")    or "1Lodn9MTb7nysq5l80cQVCu9IKfgQRlnNe754PT0hKQs"
...
class RamCache:
    ...
    def __init__(self):
        ...
        self.btn_texts: dict = {}
        self.emoji_ids: dict = {}
        self.admins: set    = set()  # qo'shimcha adminlar (super admindan tashqari)
    ...
    def from_dict(self, data: dict):
        ...
        with self._lock:
            ...
            try:
                self.admins       = {int(x) for x in (data.get("admins", []) or [])}
            except Exception:
                self.admins       = set()
            self.loaded           = True
...
def admin_menu_kb(super_admin: bool = True):
    rows = [
    ...
    if super_admin:
        rows.append([rbtn(bt("emoji_soz"), style="primary", emoji_id=get_eid("emoji_soz"))])
        rows.append([
        ...
        rows.append([rbtn(bt("broadcast"), style="danger", emoji_id=get_eid("broadcast"))])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await sm(context.bot, user.id, hello, main_menu_kb(is_admin=(is_admin(user.id))))
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "go_home":
        ...
        await sm(context.bot, uid, "Bosh menyu",
                 main_menu_kb(is_admin=(is_admin(uid))))
        return
    ...
    if data.startswith("finish_movie|"):
        ...
        if ok:
            await sm(context.bot, uid,
                ...
                admin_menu_kb(super_admin=is_super_admin(uid)))
        else:
            await sm(context.bot, uid,
...
async def cb_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "bc_cancel":
        ...
        await sm(context.bot, uid, "Admin panel", admin_menu_kb(super_admin=is_super_admin(uid)))
        return
...
def _get_admin_reserved_texts() -> set:
    keys = [
        ...
        "barcha_kino", "kino_kanal_set", "admin_qosh",
    ]
    result = set()
    for k in keys:
        v = bt(k)
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── 1. editing_btn_key ─────────────────────────────
    if is_admin(uid) and context.user_data.get("editing_btn_key"):
    ...
    if is_admin(uid) and context.user_data.get("bc_adding_btn"):
    ...
    # ── 3. Emoji menyu ──────────────────────────────────
    if is_admin(uid) and context.user_data.get("emoji_menu"):
    ...
    # ── 4. Kanal boshqarish ─────────────────────────────
    if is_admin(uid) and context.user_data.get("channel_manage_menu"):
    ...
    # ── 5. Admin reply_to ───────────────────────────────
    if is_admin(uid) and "reply_to" in context.user_data:
    ...
    if is_admin(uid) and context.user_data.get("admin_state"):
        ...
        if nav_key:
            ...
            if nav_key == "asosiy":
                ...
                await sm(context.bot, uid, "<b>Admin panel</b>", admin_menu_kb(super_admin=is_super_admin(uid)))
    ...
    if is_admin(uid):
        all_admin_btn_keys = [
            ...
            "kino_kanal_set", "admin_qosh",
        ]
        all_admin_btns = {bt(k): k for k in all_admin_btn_keys if bt(k)}
        if text in all_admin_btns:
            key = all_admin_btns[text]
            # Faqat super-admin uchun ruxsat etilgan tugmalar
            if key in ("emoji_soz", "kino_uch", "admin_qosh") and not is_super_admin(uid):
                await sm(context.bot, uid, "❌ Bu amal faqat asosiy admin uchun.")
                return
            if key == "admin_qosh":
                clear_admin_state(context)
                context.user_data["admin_state"] = "add_admin_id"
                cur = ", ".join(str(x) for x in sorted(RAM.admins)) or "(yo'q)"
                await sm(context.bot, uid,
                    "👮 <b>Admin qo'shish</b>\n\n"
                    f"Hozirgi qo'shimcha adminlar: <code>{cur}</code>\n\n"
                    "Yangi admin <b>Telegram ID</b> sini kiriting:")
                return
            if key == "emoji_soz":
                clear_admin_state(context)
                context.user_data["emoji_menu"] = True
...
async def admin_state_handler(update, context, text: str) -> bool:
    ...
    if state == "add_admin_id":
        if not is_super_admin(uid):
            context.user_data.pop("admin_state", None)
            await sm(context.bot, uid, "❌ Faqat asosiy admin admin qo'sha oladi.", admin_menu_kb(super_admin=False))
            return True
        raw = text.strip()
        if not raw.lstrip("-").isdigit():
            await sm(context.bot, uid, "❌ Faqat raqam (Telegram ID) yuboring. Qayta kiriting:")
            return True
        new_id = int(raw)
        if new_id == ADMIN_ID:
            await sm(context.bot, uid, "ℹ️ Bu ID allaqachon asosiy admin.", admin_menu_kb(super_admin=True))
            context.user_data.pop("admin_state", None)
            return True
        if new_id in RAM.admins:
            await sm(context.bot, uid, f"ℹ️ <code>{new_id}</code> allaqachon admin.", admin_menu_kb(super_admin=True))
            context.user_data.pop("admin_state", None)
            return True
        RAM.admins.add(new_id)
        await save_now()
        context.user_data.pop("admin_state", None)
        await sm(context.bot, uid,
            f"✅ <code>{new_id}</code> admin sifatida qo'shildi!\n\n"
            f"Jami qo'shimcha adminlar: <b>{len(RAM.admins)}</b>",
            admin_menu_kb(super_admin=True))
        try:
            await sm(context.bot, new_id,
                "🎉 Sizga <b>admin</b> huquqi berildi!\n/start bosing.")
        except Exception as e:
            logger.warning(f"Yangi adminga xabar yuborilmadi: {e}")
        return True
    ...
    if state == "set_card":
        ...
        await save_now()
        context.user_data.pop("admin_state", None)
        await sm(context.bot, uid, f"✅ Karta saqlandi: <code>{text}</code>", admin_menu_kb(super_admin=is_super_admin(uid)))
        return True
    ...
    if state == "set_kino_kanal":
        ...
        if text.strip() == "0":
            ...
            await save_now()
            await sm(context.bot, uid, "✅ Kino kodlari kanali linki <b>o'chirildi</b>!", admin_menu_kb(super_admin=is_super_admin(uid)))
    ...
    if state == "add_movie_poster":
        ...
        if code and code in RAM.movies:
            ...
            await sm(context.bot, uid, "✅ Kino qo'shildi!", admin_menu_kb(super_admin=is_super_admin(uid)))
        return True
    ...
    if state == "set_price_amount":
        ...
        if amount == "0":
            ...
            await save_now()
            await sm(context.bot, uid,
                f"✅ <b>{movie_title}</b> — <b>{ep}-qism</b> endi <b>bepul</b>!", admin_menu_kb(super_admin=is_super_admin(uid)))
        else:
    ...
    if state == "post_channel_target":
        ...
        try:
            ...
            await sm(context.bot, uid, "✅ Post yuborildi!", admin_menu_kb(super_admin=is_super_admin(uid)))
        except Exception as e:
            await sm(context.bot, uid, f"❌ Xato: {e}")
        return True
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(uid) and state == "broadcast_msg":
        bc = {
            "type": "copy",
    ...
    if is_admin(uid) and state == "add_movie_poster":
        code = context.user_data.pop("poster_code", None)
        context.user_data.pop("admin_state", None)
        context.user_data.pop("new_movie_code", None)
    ...
    if is_admin(uid) and state == "add_ep_video":
    ...
    if is_admin(uid) and state == "set_install":
        if msg.video:
            RAM.settings["install_video_id"] = msg.video.file_id
            await save_now()
            context.user_data.pop("admin_state", None)
            await sm(context.bot, uid, "✅ O'rnatish videosi saqlandi!", admin_menu_kb(super_admin=is_super_admin(uid)))
        elif msg.document:
            RAM.settings["install_file_id"] = msg.document.file_id
            await save_now()
            context.user_data.pop("admin_state", None)
            await sm(context.bot, uid, "✅ O'rnatish fayli saqlandi!", admin_menu_kb(super_admin=is_super_admin(uid)))
        else:
    ...
    if context.user_data.get("awaiting_check") and msg.photo:
        ...
        await sm(context.bot, uid,
            ...
            "👨‍💼 Admin tekshirib tasdiqlaydi va shu qismni videosini sizga yuboradi.\n"
