...
def ibtn(text, data=None, url=None, style=None, emoji_id=None):
    # Faqat Telegram Bot API qo'llaydigan maydonlar yuboriladi.
    # style/icon_custom_emoji_id kabi ortiqcha maydonlar tugmalar ishlamasligiga sabab bo'ladi.
    b = {"text": button_text(text)}
    if data: b["callback_data"] = data
    if url:  b["url"] = url
    return b

def rbtn(text, style=None, emoji_id=None):
    return {"text": button_text(text)}
