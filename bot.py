...
ADMIN_PERM_KEYS = [
    ...
    "qism_tahrir", "kino_uch", "premium", "admin_qosh", "tugma_rang",
]
...
DEFAULT_BTN = {
    ...
    "tugma_rang":     _B('Tugma rangini ozgartirish'),
}
...
BTN_LABELS["tugma_rang"]   = "Tugma rangini o'zgartirish"
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = q.data or ""
    uid  = q.from_user.id
    # NOTE: q.answer() bcol_* handlerlarda show_alert bilan chaqiriladi,
    # shuning uchun bu yerda chaqirmaymiz (aks holda alert chiqmaydi).
    if not data.startswith("bcol_"):
        try: await q.answer()
        except Exception: pass
...
async def admin_buttons(update, context, text: str):
    ...
    if text == bt("tugma_rang"):
        if not (is_super_admin(uid) or has_perm(uid, "tugma_rang")):
            await sm(context.bot, uid, "⛔ Sizda bu amalga ruxsat yo'q.", admin_menu_kb(uid))
            return
        await sm(context.bot, uid,
            "🎨 <b>Tugma rangini o'zgartirish</b>\n\n"
            "Pastdan tugmani tanlang — keyin 3 ta rangdan birini tanlaysiz.",
            color_pick_list_kb(0))
        return
