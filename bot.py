...
class RamCache:
    ...
    def __init__(self):
        ...
        self.btn_texts: dict = {}
        self.emoji_ids: dict = {}
        self.sub_admins: dict = {}   # {uid_str: {"perms": {key: bool}, "added_by": str}}
        self.btn_styles: dict = {}   # {btn_key: "primary"|"success"|"danger"}
    ...
    def to_dict(self) -> dict:
        with self._lock:
            return {
                ...
                "btn_styles":       copy.deepcopy(self.btn_styles),
            }
    ...
    def from_dict(self, data: dict):
        ...
        with self._lock:
            ...
            self.btn_styles       = data.get("btn_styles", {}) or {}
            self.loaded           = True
...
ADMIN_PERM_KEYS = [
    ...
    "maj_kanal", "karta", "kino_kanal_set",
...
# ── Tugma rangi (admin tomonidan o'zgartiriladi) ──
ALLOWED_STYLES = ("primary", "success", "danger")
STYLE_LABELS = {"primary": "🔵 Kok", "success": "🟢 Yashil", "danger": "🔴 Qizil"}
def get_btn_style(key: str, default: str = "primary") -> str:
    try:
        v = (RAM.btn_styles or {}).get(key)
        if v in ALLOWED_STYLES:
            return v
    except Exception:
        pass
    return default

def set_btn_style(key: str, style: str):
    if style not in ALLOWED_STYLES: return
    if key not in BTN_LABELS: return
    RAM.btn_styles[key] = style

# ── Admin ierarxiya yordamchilari ──
def admin_added_by(target_uid) -> str:
    sub = (RAM.sub_admins or {}).get(str(target_uid)) or {}
    return str(sub.get("added_by") or "")

def can_manage_admin(actor_uid, target_uid) -> bool:
    if is_super_admin(actor_uid):
        return True
    if not is_any_admin(actor_uid):
        return False
    return admin_added_by(target_uid) == str(actor_uid)
...
JSONBLOB_DEBOUNCE = 2.0   # soniya — har o'zgarish bazaga tezroq yozilsin
...
for _i, _ch in enumerate("ABCDEFGHIJKLMNOPQRSTUVWXYZ"):
    _MATH_BOLD_MAP[chr(0x1D5D4 + _i)] = _ch
for _i, _ch in enumerate("abcdefghijklmnopqrstuvwxyz"):
    _MATH_BOLD_MAP[chr(0x1D5EE + _i)] = _ch
for _i, _ch in enumerate("0123456789"):
    _MATH_BOLD_MAP[chr(0x1D7EC + _i)] = _ch

def _plain_button_text(text: str) -> str:
    text = "".join(_MATH_BOLD_MAP.get(ch, ch) for ch in str(text or ""))
    text = re.sub(r"<[^>]+>", "", text)
...
def button_text(text: str) -> str:
    # Telegram tugma shriftini kattalashtirib bo'lmaydi, shuning uchun
    # barcha tugma yozuvlari katta harf + qalin Unicode qilib beriladi.
    plain = _plain_button_text(text).upper()
    return _B(plain)

def bt(key: str) -> str:
    raw = RAM.btn_texts.get(key) or DEFAULT_BTN.get(key, "")
    return button_text(raw)


def get_eid(key: str):
    # Premium/custom emoji tugma ikonkalari o'chirildi: Telegram API'da
    # ayrim botlarda tugmalar bosilmay qolishiga sabab bo'layotgan edi.
    return None
...
def ibtn(text, data=None, url=None, style=None, emoji_id=None):
    # Telegram Bot API tugmalarda rangni qo'llab-quvvatlamaydi.
    # Shuning uchun "rang" tanlovi vizual ravishda matn oldidagi rangli emoji bilan ko'rsatiladi.
    prefix = ""
    if style == "primary": prefix = "🔵 "
    elif style == "success": prefix = "🟢 "
    elif style == "danger":  prefix = "🔴 "
    b = {"text": prefix + button_text(text)}
...
def rbtn(text, style=None, emoji_id=None):
    ...
    return {"text": prefix + button_text(text)}
...
def main_menu_kb(is_admin=False):
    rows = [
        [
            rbtn(bt("yordam"),      style=get_btn_style("yordam",      "primary"), emoji_id=get_eid("yordam")),
            rbtn(bt("barcha_kino"), style=get_btn_style("barcha_kino", "primary"), emoji_id=get_eid("barcha_kino")),
        ],
    ]
    if is_admin:
        rows.append([rbtn(bt("boshqarish"), style=get_btn_style("boshqarish", "primary"), emoji_id=get_eid("boshqarish"))])
    return rkb(rows)
...
def admin_menu_kb(uid=None):
    ...
    rows, buf = [], []
    for k, st in pairs:
        buf.append(rbtn(bt(k), style=get_btn_style(k, st), emoji_id=get_eid(k)))
        if len(buf) == 2:
            rows.append(buf); buf = []
    ...
    if uid is None or is_super_admin(uid) or has_perm(uid, "admin_qosh"):
        rows.append([rbtn(bt("admin_qosh"), style=get_btn_style("admin_qosh", "success"), emoji_id=get_eid("admin_qosh"))])
    rows.append([rbtn(bt("asosiy"), style=get_btn_style("asosiy", "success"), emoji_id=get_eid("asosiy"))])
    return rkb(rows)


def channel_manage_kb():
    return rkb([
        [rbtn(bt("kanal_qosh"),   style=get_btn_style("kanal_qosh",   "success"), emoji_id=get_eid("kanal_qosh")),
         rbtn(bt("kanal_uch"),    style=get_btn_style("kanal_uch",    "danger"),  emoji_id=get_eid("kanal_uch"))],
        [rbtn(bt("kanal_royxat"), style=get_btn_style("kanal_royxat", "primary"), emoji_id=get_eid("kanal_royxat"))],
        [rbtn(bt("admin_panel"),  style=get_btn_style("admin_panel",  "success"), emoji_id=get_eid("admin_panel"))],
    ])
...
def payment_sent_kb(card: str = "", price: int = 0):
    ...
    if card:
        copy_row.append({
            "text": button_text(bt("karta_nusxa")),
            "copy_text": {"text": str(card)},
        })
    if price:
        copy_row.append({
            "text": button_text(bt("miqdor_nusxa")),
...
# ── Tugma rangini o'zgartirish: inline klaviaturalar ──
def color_pick_list_kb(page: int = 0, per_page: int = 10):
    keys = list(BTN_LABELS.keys())
    total = len(keys)
    ...
    for k in keys[start:end]:
        cur = get_btn_style(k, "primary")
        mark = STYLE_LABELS.get(cur, "").split(" ")[0]
        label = BTN_LABELS.get(k, k)
        rows.append([ibtn(f"{mark} {label}", data=f"bcol_pick|{k}|{page}", style=cur)])
    nav = []
    if page > 0:
        nav.append(ibtn("⬅️", data=f"bcol_page|{page-1}", style="primary"))
    nav.append(ibtn(f"{page+1}/{pages}", data="bcol_noop", style="success"))
    if page < pages - 1:
        nav.append(ibtn("➡️", data=f"bcol_page|{page+1}", style="primary"))
    rows.append(nav)
    rows.append([ibtn("♻️ Hammasini default", data="bcol_reset_all", style="danger")])
    rows.append([ibtn("❌ Yopish", data="bcol_close", style="danger")])
    return ikb(rows)

def color_choice_kb(key: str, page: int = 0):
    cur = get_btn_style(key, "primary")
    rows = [[
        ibtn(("✅ " if cur=="primary" else "")+"🔵 Kok",    data=f"bcol_set|{key}|primary|{page}", style="primary"),
        ibtn(("✅ " if cur=="success" else "")+"🟢 Yashil", data=f"bcol_set|{key}|success|{page}", style="success"),
        ibtn(("✅ " if cur=="danger" else "")+"🔴 Qizil",  data=f"bcol_set|{key}|danger|{page}",  style="danger"),
    ]]
    rows.append([ibtn("⬅️ Orqaga", data=f"bcol_page|{page}", style="primary")])
    return ikb(rows)

def sub_admin_perm_kb(target_uid: str):
    """Sub-admin uchun perm toggle inline kb."""
    sub = RAM.sub_admins.get(str(target_uid), {}) or {}
    perms = sub.get("perms", {}) or {}
    rows, buf = [], []
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = q.data or ""
    uid  = q.from_user.id
    # Eski ortiqcha emoji/rang sozlama tugmalari o'chirildi.
    if data.startswith("emoji_") or data.startswith("bcol_"):
        try: await q.answer("Bu ortiqcha sozlama o'chirilgan.", show_alert=True)
        except Exception: pass
    ...
    if data == "bcol_noop":
        ...
        return
    if data == "bcol_reset_all":
        if not (is_super_admin(uid) or has_perm(uid, "tugma_rang")):
            try: await q.answer("⛔ Ruxsat yo'q", show_alert=True)
            except: pass
            return
        RAM.btn_styles = {}
        try: await q.edit_message_text("🎨 <b>Tugma rangi</b>\n\nIstalgan tugmani tanlang:",
                                       parse_mode="HTML", reply_markup=color_pick_list_kb(0))
        except Exception: pass
        try: await q.answer("♻️ Hammasi default")
        except: pass
        asyncio.create_task(asyncio.to_thread(save_now_sync)) if False else None
        try:
            await save_now()
        except Exception: pass
        return
    if data.startswith("bcol_page|"):
        if not (is_super_admin(uid) or has_perm(uid, "tugma_rang")): return
        try:
            page = int(data.split("|", 1)[1])
        except Exception:
            page = 0
        try:
            await q.edit_message_text("🎨 <b>Tugma rangi</b>\n\nIstalgan tugmani tanlang:",
                                      parse_mode="HTML", reply_markup=color_pick_list_kb(page))
        except Exception:
            try: await q.edit_message_reply_markup(reply_markup=color_pick_list_kb(page))
            except: pass
        try: await q.answer()
        except: pass
        return
    if data.startswith("bcol_pick|"):
        if not (is_super_admin(uid) or has_perm(uid, "tugma_rang")): return
        try:
        ...
        if key not in BTN_LABELS:
            try: await q.answer("❌ Notog'ri tugma", show_alert=True)
            except: pass
            return
        label = BTN_LABELS.get(key, key)
        cur = get_btn_style(key, "primary")
        try:
            await q.edit_message_text(
                f"🎨 <b>{label}</b>\nHozirgi rang: {STYLE_LABELS.get(cur, cur)}\n\nYangi rangni tanlang:",
                parse_mode="HTML", reply_markup=color_choice_kb(key, page))
        except Exception:
            try: await q.edit_message_reply_markup(reply_markup=color_choice_kb(key, page))
            except: pass
        try: await q.answer()
        except: pass
        return
    if data.startswith("bcol_set|"):
        if not (is_super_admin(uid) or has_perm(uid, "tugma_rang")):
            try: await q.answer("⛔ Ruxsat yo'q", show_alert=True)
            except: pass
            return
        try:
            _, key, color, page = data.split("|", 3)
            page = int(page)
        except Exception:
            return
        if key not in BTN_LABELS or color not in ALLOWED_STYLES:
            try: await q.answer("❌ Xato", show_alert=True)
            except: pass
            return
        set_btn_style(key, color)
        try: await save_now()
        except Exception: pass
        try:
            _lbl = BTN_LABELS.get(key, key)
            _stl = STYLE_LABELS.get(color, color)
            await q.answer(f"✅ Muvaffaqiyatli!\n«{_lbl}» tugmasi {_stl} rangga o'zgardi", show_alert=True)
        except Exception: pass
        try: pass
        except: pass
        try:
    ...
    if data.startswith("adm_perm|"):
        try:
            _, target, _key = data.split("|", 2)
        except Exception:
            return
        if not can_manage_admin(uid, target): return
        # Sub-admin (level-1) o'zining qo'shganlariga "admin_qosh" perm bera olmaydi
        if not is_super_admin(uid) and _key == "admin_qosh": return
        try:
    ...
    if data.startswith("adm_del|"):
        try:
            ...
            return
        if not can_manage_admin(uid, _t): return
        try:
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── 1. Eski emoji/tugma matni sozlamalari o'chirilgan ─────
    if is_any_admin(uid) and (context.user_data.get("editing_btn_key") or context.user_data.get("emoji_menu")):
        context.user_data.pop("editing_btn_key", None)
        context.user_data.pop("emoji_menu", None)
        await sm(context.bot, uid, "Bu ortiqcha sozlama o'chirilgan.", admin_menu_kb(uid))
        return
    ...
    # ── 3. Eski emoji menyu o'chirilgan ──────────────────
    ...
    if is_any_admin(uid):
        all_admin_btn_keys = [
            ...
            "asosiy", "boshqarish", "broadcast", "kino_uch",
...
async def admin_buttons(update, context, text: str):
    ...
    if text == bt("admin_qosh"):
        ...
        if is_super_admin(uid):
            # Bosh admin — barcha adminlarni va kim qo'shganini ko'radi
            lines = []
            for u, info in cur.items():
                ab = (info or {}).get("added_by")
                if ab and str(ab) != str(ADMIN_ID):
                    lines.append(f"• <code>{u}</code>  <i>(qo'shgan: <code>{ab}</code>)</i>")
                else:
                    lines.append(f"• <code>{u}</code>")
            cur_list = "\n".join(lines) or "<i>Hozircha yo'q</i>"
        else:
            # Sub-admin — faqat o'zi qo'shganlarini ko'radi
            mine = [u for u, info in cur.items() if str((info or {}).get("added_by") or "") == str(uid)]
            cur_list = "\n".join([f"• <code>{u}</code>" for u in mine]) or "<i>Siz hali admin qo'shmagansiz</i>"
        await sm(context.bot, uid,
            "👮 <b>Admin qo'shish</b>\n\n"
            f"Sizning adminlaringiz:\n{cur_list}\n\n"
...
async def admin_state_handler(update, context, text: str) -> bool:
    ...
    if state == "add_admin_id":
        ...
        if raw.startswith("-") and raw[1:].isdigit():
            target = raw[1:]
            if target not in (RAM.sub_admins or {}):
                await sm(context.bot, uid, f"❌ <code>{target}</code> admin emas.")
                return True
            if not can_manage_admin(uid, target):
                await sm(context.bot, uid, "⛔ Siz faqat o'zingiz qo'shgan adminni o'chira olasiz.")
                clear_admin_state(context)
                return True
            RAM.sub_admins.pop(target, None)
            await save_now()
        ...
        # Sub-admin (level-1) faqat o'zi qo'shgan adminni boshqara oladi (yoqib/o'chirib qo'yadi).
        if target not in RAM.sub_admins:
            default_perms = {k: True for k in ADMIN_PERM_KEYS}
            if not is_super_admin(uid):
                default_perms["admin_qosh"] = False
            RAM.sub_admins[target] = {"perms": default_perms, "added_by": str(uid)}
        else:
            # mavjud admin — added_by yo'q bo'lsa, hozirgi qo'shuvchini saqlab qo'yamiz (faqat super)
            if is_super_admin(uid):
                RAM.sub_admins[target].setdefault("added_by", str(uid))
        await save_now()
        clear_admin_state(context)
        await sm(context.bot, uid,
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_any_admin(uid) and state == "add_ep_video":
        ...
        await save_ram_only()
        # JSONBlob bazasiga ham darhol fon rejimida yozamiz — reset bo'lsa yo'qolmasin.
...
def main():
    ...
    async def _periodic_sync(context_job):
        try:
            if DB_STATUS.get("load_failed"):
                blob = await asyncio.to_thread(_load_jsonblob)
                if isinstance(blob, dict):
                    merged = _merge_db(blob, RAM.to_dict())
                    RAM.from_dict(merged)
                    DB_STATUS["load_failed"] = False
                    logger.info("✅ JSONBlob qayta yuklandi va RAM bilan birlashtirildi")
            data    = RAM.to_dict()
